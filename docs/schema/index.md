...
## Las cuatro cosas que más caro cuesta ignorar
...
4. **Comparar fechas con `'YYYY-MM-DD'` puede invertir día/mes.** Con `DATEFORMAT dmy` de sesión (default en español), si el día es ≤ 12 la query corre sin error pero filtra la fecha equivocada. Usar siempre `'YYYYMMDD'` sin separadores. Ver [Calidad de datos](calidad-de-datos.md#comparar-fechas-con-separadores-puede-invertir-diames-dateformat).
